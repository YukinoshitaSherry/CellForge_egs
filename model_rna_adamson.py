...
class PerturbationEncoder(nn.Module):
    def __init__(self, pert_dim, hidden_dim, dropout=0.1):
        super().__init__()
    ...
    def forward(self, pert):
        ...
        output = self.encoder(pert_emb)
        return output

class StatePerturbationCrossAttention(nn.Module):
    def __init__(self, state_dim, pert_dim, hidden_dim, n_heads=8, dropout=0.1):
        super().__init__()
    ...
    def forward(self, z_state, p_emb):
        batch_size = z_state.size(0)
        z_state_proj = self.state_proj(z_state).unsqueeze(1)
        p_emb_expanded = p_emb.unsqueeze(1)
        residual = z_state_proj
        Q = self.query_proj(z_state_proj).view(batch_size, 1, self.n_heads, self.head_dim).transpose(1, 2)
        K = self.key_proj(p_emb_expanded).view(batch_size, 1, self.n_heads, self.head_dim).transpose(1, 2)
        V = self.value_proj(p_emb_expanded).view(batch_size, 1, self.n_heads, self.head_dim).transpose(1, 2)
...
class GALAModel(nn.Module):
    def __init__(self, input_dim, output_dim, pert_dim, hidden_dim=256, n_layers=2, n_heads=8,
        ...
        self.cross_attention = StatePerturbationCrossAttention(
    ...
    def forward(self, x_control, pert):
        ...
        z_state = self.expression_encoder(x_control)
        p_emb = self.pert_encoder(pert)
        h_int = self.cross_attention(z_state, p_emb)
        h_int = h_int + z_state
        x_trans = self.mlp(h_int)
        output = self.output_head(x_trans)
        return output
...
def evaluate_model(model, test_loader, device, max_pert_dim=None, test_dataset=None):
    model.eval()
    total_loss = 0
    all_targets = []
    all_predictions = []
...
def objective(trial):
    ...
    patience = 3
    patience_counter = 0
    for epoch in tqdm(range(3), desc="Hyperparameter Trial"):
        train_loss = train_model(model, train_loader, optimizer, scheduler, device, max_pert_dim=pert_dim)
        val_metrics = evaluate_model(model, val_loader, device, max_pert_dim=pert_dim)
...
def main(gpu_id=None):
    ...
    train_path = "/datasets/AdamsonWeissman2016_train_processed.h5ad"
    test_path = "/datasets/AdamsonWeissman2016_test_processed.h5ad"
    ...
    test_metrics = evaluate_model(model, test_loader, device,
                                  max_pert_dim=pert_dim, test_dataset=test_dataset)
    ...
    results_df.to_csv(f'adamson_weissman_evaluation_results_{timestamp}.csv', index=False)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='GALA-Former Model: AdamsonWeissman2016')
    parser.add_argument('--gpu', type=int, default=None, help='GPU ID to use')
    args = parser.parse_args()
    print("=" * 60)
    print("GALA-Former Model Training for AdamsonWeissman2016")
    print("=" * 60)
