"""
Cytokine Trajectory Model for Single-Cell Perturbation Prediction
Based on SchiebingerLander2019 dataset

This script implements a Trajectory-Aware VAE with Perturbation Mixing
for predicting single-cell transcriptional responses to cytokine perturbations.
"""
...
class CytokineTrajectoryDataset(Dataset):
    """Dataset for single-cell perturbation prediction with temporal information"""
    ...
    def __init__(self, adata, perturbation_key='perturbation', age_key='age', 
        ...
        if pca_model is None:
            if fit_pca:
                if not is_train:
                    raise ValueError('Test set must use training set PCA model. pca_model must be provided.')
    ...
    def _encode_timepoints(self, timepoints):
        """Encode timepoints as numerical features"""
        time_mapping = {}
...
class CytokineTrajectoryModel(nn.Module):
    ...
    def forward(self, x, pert, time_emb):
        ...
        z, shared_mu, shared_logvar, condition_mu, condition_logvar = self.encoder(x, time_emb)
...
def train_model(model, train_loader, optimizer, scheduler, device, 
    ...
    for i, batch in enumerate(train_loader):
        ...
        x_baseline, pert, time_emb, x_target = x_baseline.to(device), pert.to(device), time_emb.to(device), x_target.to(device)
        ...
        recon_loss = F.mse_loss(outputs['predicted_expr'], x_target)
...
def evaluate_model(model, test_loader, device, aux_weight=0.1, contrastive_weight=0.05, ot_weight=0.01):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            x_pred_np = outputs['predicted_expr'].cpu().numpy()
...
def evaluate_and_save_model(model, test_loader, device, save_path, 
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc='Evaluating'):
            ...
            all_predictions.append(outputs['predicted_expr'].cpu().numpy())
