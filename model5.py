...
def objective(trial, timestamp):
    ...
    if test_adata is not None:
        ...
        # Fallback: get perturbation names from test_dataset if available
        if test_dataset is not None and hasattr(test_dataset, 'perturbation_names'):
            ...
            # Last resort: use train perturbation names (for unseen perturbation, this is acceptable)
...
if __name__ == '__main__':
    ...
    results_df = main(gpu_id=args.gpu)import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np
import scanpy as sc
